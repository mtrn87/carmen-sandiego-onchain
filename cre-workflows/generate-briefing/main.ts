 ...
 *    This workflow is compiled to WASM and executed by the Chainlink DON
 *    (Decentralized Oracle Network). When a player starts a mission,
 *    the MissionStarted event triggers this workflow across multiple
 *    independent Chainlink nodes. Each node generates the same briefing,
 *    the DON reaches consensus, and the signed result is written on-chain.
 *
 *    WHY DECENTRALIZED:
 *    - The briefing content is deterministic (based on missionId and
 *      scenario data), ensuring all DON nodes produce identical output.
 *    - No centralized server generates or controls narrative content.
 *    - The DON's threshold signature guarantees authenticity — only
 *      briefings produced by the authorized CRE workflow are accepted
 *      by the GameMasterProxy contract on-chain.
 *    - End-to-end encryption (ECIES) means even the DON nodes cannot
 *      read the briefing after encryption — only the player can decrypt.
 *
 *  DATA FLOW:
 *    1. Player calls startMission() on GameMaster.sol
 *    2. VRF v2.5 fulfills randomness, GameMaster emits MissionStarted
 *    3. CRE DON detects the event via LogTrigger capability
 *    4. DON nodes execute this WASM workflow:
 *       a. Read player's ECIES public key from on-chain registry
 *       b. Read mission salt and valid cities
 *       c. Select scenario based on missionId (deterministic rotation)
 *       d. Build enriched noir-style briefing with city/chain details
 *       e. ECIES-encrypt briefing with player's secp256k1 public key
 *       f. Compute keccak256 content hash for on-chain verification
 *    5. DON consensus produces a signed Keystone report
 *    6. Report delivered to GameMasterProxy via writeReport()
 *    7. Proxy verifies signature and forwards to GameMaster.receiveClue()
 *
 *  CHAINLINK SERVICES USED:
 *    - CRE/Keystone: WASM execution, LogTrigger, consensus, signed reports
 *    - EVMClient: Reads player public key and mission state from Sepolia
 *    - writeReport: On-chain delivery via Keystone Forwarder
 *    - VRF v2.5: The mission salt (used for scenario selection) was
 *      generated by Chainlink VRF in the preceding startMission() call
 *
