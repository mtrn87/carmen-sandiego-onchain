...
describe("PlayerRegistry - Manual Tests", function () {
  ...
  describe("Fluxo Completo: Login", function () {
    it("Should handle complete login flow", async function () {
      ...
      // Step 3: Frontend calls requestRegistration (triggers CRE event)
      console.log("Step 3: Frontend chama requestRegistration (emite evento para CRE)");
      const tx = await playerRegistry.connect(addr1).requestRegistration(nickname);
      const receipt = await tx.wait();
      console.log("  TX hash:", tx.hash);
      console.log("  Block:", receipt?.blockNumber);
      console.log("  ✓ Evento RegistrationRequested emitido para CRE\n");

      // Step 4: CRE processes → GameMaster calls registerPlayer
      console.log("Step 4: CRE processa → GameMaster registra o jogador");
      const tx2 = await playerRegistry.connect(gameMaster).registerPlayer(addr1.address, nickname);
      await tx2.wait();
      console.log("  ✓ GameMaster registrou jogador com sucesso\n");
